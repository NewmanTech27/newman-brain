...
Note (scope guard): the pgTAP `db-tests` job, which applies `supabase/migrations/` from
an empty database, **passed** on `dev@a81c43c` — so dev's 75 files are internally
consistent and replay cleanly *in isolation*. The drift is specifically **git-versions vs
the hosted-prod ledger**, not a broken local build. Related but distinct:
[[main-dev-fork]] and the CRM-only-schema gap.
